...
def preprocess(param_file_name="params/param.yaml"):
    try:
        ...
        for i in range(len(X_train)):
            ...
            dest_path = os.path.join(train_folder, path)
            if not os.path.exists(os.path.dirname(dest_path)):
                os.makedirs(os.path.dirname(dest_path))
            if not os.path.exists(dest_path):
                shutil.copyfile(X_train[i], dest_path)
            else:
                print(f"Image '{X_train[i]}' already exists in the destination folder.")
            new_row = pd.DataFrame({'path': [path], 'label': [y_train[i]], 'is_train': [True]})
            df = pd.concat([df, new_row], ignore_index=True)

        for i in range(len(X_test)):
            path = os.path.relpath(X_test[i], params['lake_dir'])
            dest_path = os.path.join(val_folder, path)
            if not os.path.exists(os.path.dirname(dest_path)):
                os.makedirs(os.path.dirname(dest_path))
            if not os.path.exists(dest_path):
                shutil.copyfile(X_test[i], dest_path)
            else:
                print(f"Image '{X_test[i]}' already exists in the destination folder.")
            new_row = pd.DataFrame({'path': [path], 'label': [y_test[i]], 'is_train': [True]})
            df = pd.concat([df, new_row], ignore_index=True)
...
preprocess('/mnt/d/ML/MLOps Basic Pipeline for Flower dataset/pipelines/dags/data/params/param.yaml')